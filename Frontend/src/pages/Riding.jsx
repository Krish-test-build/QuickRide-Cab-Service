...
const fare = location.state?.fare;
  ...
  return (
    <div className="h-screen relative">
      <div>
        <Link to="/home" className="z-[50] absolute h-11 w-11 top-3 right-4 m-3 bg-[#ededed] rounded-full">
          <svg className="h-8 w-8 mt-1.5 ml-1.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
      ...
      <img className="w-30 h-auto left-5 mt-0 absolute z-[50]" src={icon} />
          ...
          <div className="w-full max-w-md">
            <div className="flex justify-between items-start mb-4">
              <div className="relative w-32 h-24">
                <img
                  ...
                  className={`${ride?.vehicleType === 'auto' ? 'w-[85%] ml-5' : 'w-full'} h-auto rounded-lg`}
                />
              ...
              <div className="flex flex-col items-end">
                <span className="text-base capitalize">{ride?.captain?.fullName?.firstName} {ride?.captain?.fullName?.lastName}</span>
                <span className="text-2xl font-semibold">{ride?.captain?.vehicle?.plate}</span>
            ...
            <div className="bg-[#f9f9f9] rounded-xl p-4 mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-base">{ride?.destination}</span>
              </div>
            </div>

            <button className="w-full bg-black text-white py-3 rounded-lg font-medium">
              Make a Payment of ₹{ride?.fare || fare?.[ride?.vehicleType]}
